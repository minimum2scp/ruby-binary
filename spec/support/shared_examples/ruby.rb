RSpec.shared_examples 'ruby 3.3.5' do
  ...
  describe command("RBENV_VERSION=3.3.5 gem -v") do
    let(:login_shell){ true }
    its(:stdout){ should eq "3.5.22\n" }
  end

  describe command("RBENV_VERSION=3.3.5 gem list --exact bundler") do
    let(:login_shell){ true }
    its(:stdout){ should eq "bundler (default: 2.5.22)\n" }
  end
...
RSpec.shared_examples 'ruby 3.2.5' do
  describe command("RBENV_VERSION=3.2.5 ruby -v") do
    let(:login_shell){ true }
    its(:stdout){ should eq "ruby 3.2.5 (2024-07-26 revision 31d0f1a2e7) [x86_64-linux]\n" }
  end

  describe command("RBENV_VERSION=3.2.5 gem -v") do
    let(:login_shell){ true }
    its(:stdout){ should eq "3.5.22\n" }
  end

  describe command("RBENV_VERSION=3.2.5 gem list --exact bundler") do
    let(:login_shell){ true }
    its(:stdout){ should eq "bundler (default: 2.5.22)\n" }
  end
  ...
  describe command("RBENV_VERSION=3.2.5 ruby -rrbconfig -e 'puts RbConfig::CONFIG[\"LIBRUBY_RELATIVE\"]'") do
    let(:login_shell){ true }
    its(:stdout){ should eq "yes\n" }
  end

  describe file("/opt/rbenv/versions/3.2.5/openssl") do
    it { should_not exist }
  end

  describe command("RBENV_VERSION=3.2.5 ruby -ropenssl -e 'puts OpenSSL::OPENSSL_VERSION'") do
    let(:login_shell){ true }
    its(:stdout){ should start_with("OpenSSL #{openssl_version}") }
  end

  describe command("RBENV_VERSION=3.2.5 ruby --mjit -e 'p RubyVM::MJIT.enabled?'") do
    let(:login_shell){ true }
    its(:stdout){ should eq "true\n" }
    its(:stderr){ should eq '' }
  end

  describe command("RBENV_VERSION=3.2.5 ruby --yjit -e 'p RubyVM::YJIT.enabled?'") do
    let(:login_shell){ true }
    its(:stdout){ should eq "true\n" }
    its(:stderr){ should eq '' }
  end
