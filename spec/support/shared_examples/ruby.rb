RSpec.shared_examples 'ruby 4.0.4' do
  describe command("RBENV_VERSION=4.0.4 ruby -v") do
    let(:login_shell){ true }
    its(:stdout){ should eq "ruby 4.0.4 (2026-05-12 revision b89eb1bcbf) +PRISM [x86_64-linux]\n" }
  end

  describe command("RBENV_VERSION=4.0.4 gem -v") do
    let(:login_shell){ true }
    its(:stdout){ should eq "4.0.11\n" }
  end

  describe command("RBENV_VERSION=4.0.4 gem list --exact bundler") do
    let(:login_shell){ true }
    its(:stdout){ should eq "bundler (default: 4.0.11)\n" }
  end

  describe command("RBENV_VERSION=4.0.4 ruby -rrbconfig -e 'puts RbConfig::CONFIG[\"LIBRUBY_RELATIVE\"]'") do
    let(:login_shell){ true }
    its(:stdout){ should eq "yes\n" }
  end

  describe file("/opt/rbenv/versions/4.0.4/openssl") do
    it { should_not exist }
  end

  describe command("RBENV_VERSION=4.0.4 ruby -ropenssl -e 'puts OpenSSL::OPENSSL_VERSION'") do
    let(:login_shell){ true }
    its(:stdout){ should start_with("OpenSSL #{openssl_version}") }
  end

  describe command("RBENV_VERSION=4.0.4 ruby --yjit -e 'p RubyVM::YJIT.enabled?'") do
    let(:login_shell){ true }
