RSpec.shared_examples 'ruby 3.4.1' do
  ...
  describe command("RBENV_VERSION=3.4.1 gem -v") do
    let(:login_shell){ true }
    its(:stdout){ should eq "3.6.3\n" }
  end

  describe command("RBENV_VERSION=3.4.1 gem list --exact bundler") do
    let(:login_shell){ true }
  ...
  describe file("/opt/rbenv/versions/3.4.1/openssl") do
    it { should_not exist }
  end

  describe command("RBENV_VERSION=3.4.1 ruby -ropenssl -e 'puts OpenSSL::OPENSSL_VERSION'") do
    let(:login_shell){ true }
    its(:stdout){ should start_with("OpenSSL #{openssl_version}") }
  end

  describe command("RBENV_VERSION=3.4.1 ruby --yjit -e 'p RubyVM::YJIT.enabled?'") do
    let(:login_shell){ true }
...
RSpec.shared_examples 'ruby 3.3.7' do
  describe command("RBENV_VERSION=3.3.7 ruby -v") do
    let(:login_shell){ true }
    its(:stdout){ should eq "ruby 3.3.7 (2025-01-15 revision be31f993d7) [x86_64-linux]\n" }
  end
  ...
  describe file("/opt/rbenv/versions/3.3.7/openssl") do
    it { should_not exist }
  end

  describe command("RBENV_VERSION=3.3.7 ruby -ropenssl -e 'puts OpenSSL::OPENSSL_VERSION'") do
    let(:login_shell){ true }
    its(:stdout){ should start_with("OpenSSL #{openssl_version}") }
  end

  describe command("RBENV_VERSION=3.3.7 ruby --yjit -e 'p RubyVM::YJIT.enabled?'") do
    let(:login_shell){ true }
    its(:stdout){ should eq "true\n" }
    its(:stderr){ should eq '' }
  end
...
RSpec.shared_examples 'ruby 3.2.7' do
  describe command("RBENV_VERSION=3.2.7 ruby -v") do
    let(:login_shell){ true }
    its(:stdout){ should eq "ruby 3.2.7 (2025-02-04 revision 02ec315244) [x86_64-linux]\n" }
  end
  ...
  describe file("/opt/rbenv/versions/3.2.7/openssl") do
    it { should_not exist }
  end

  describe command("RBENV_VERSION=3.2.7 ruby -ropenssl -e 'puts OpenSSL::OPENSSL_VERSION'") do
    let(:login_shell){ true }
    its(:stdout){ should start_with("OpenSSL #{openssl_version}") }
  end

  describe command("RBENV_VERSION=3.2.7 ruby --mjit -e 'p RubyVM::MJIT.enabled?'") do
    let(:login_shell){ true }
    its(:stdout){ should eq "true\n" }
    its(:stderr){ should eq '' }
  end

  describe command("RBENV_VERSION=3.2.7 ruby --yjit -e 'p RubyVM::YJIT.enabled?'") do
    let(:login_shell){ true }
    its(:stdout){ should eq "true\n" }
    its(:stderr){ should eq '' }
  end
